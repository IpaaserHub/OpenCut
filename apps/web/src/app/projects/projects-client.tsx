...
const VIEW_MODE_OPTIONS = [
	{ mode: "grid" as const, icon: GridViewIcon, label: "グリッド表示" },
	{ mode: "list" as const, icon: LeftToRightListDashIcon, label: "リスト表示" },
];
...
function NewProjectButton() {
	...
	return (
		...
		>
			...
			<span className="text-sm font-medium block md:hidden">新規</span>
		</Button>
	);
}
		...
		<Card className="bg-background overflow-hidden border-none p-0">
			...
			<CardContent className="flex flex-col gap-2 px-0 pt-4">
				...
				<div className="text-muted-foreground flex items-center gap-1.5 text-sm">
					...
					<span>作成日 {formatDate({ date: project.createdAt })}</span>
				</div>
			</CardContent>
		</Card>
	);
			...
			<div className="flex flex-col items-center justify-center gap-5 py-16 text-center">
				<div className="flex flex-col items-center gap-8">
					...
					<div className="flex flex-col items-center gap-3">
						<h3 className="text-lg font-medium">検索結果がありません</h3>
						<p className="text-muted-foreground max-w-md">
							「{searchQuery}」に一致するプロジェクトは見つかりませんでした。
						</p>
					</div>
				</div>
				<Button
	...
	return (
		<div className="flex flex-col items-center justify-center gap-6 py-16 text-center">
			<div className="flex flex-col items-center gap-2">
				...
				<h3 className="text-lg font-medium">まだプロジェクトがありません</h3>
				<p className="text-muted-foreground max-w-md">
					最初のプロジェクトを作成しましょう。メディアを読み込み、編集して、動画を書き出せます。
				</p>
			</div>
